...
@router.get("/games/top-favorites", response_model=List[GameResponse])
def get_top_5_favorite_games():
    """
    Get the top 5 most favorited games.

    Returns the top 5 most favorited games across all user profiles, sorted by popularity and then alphabetically.
    """
    with DatabaseSession(dict_cursor=True) as db:
        db.execute(
            """
            SELECT 
                ...
                game.name,
                game.category,
                COUNT(uGame.id) as favorite_count
            FROM games game
            INNER JOIN user_games uGame ON game.id = uGame.game_id
            WHERE uGame.is_favorite = TRUE
            GROUP BY game.id, game.name, game.category
